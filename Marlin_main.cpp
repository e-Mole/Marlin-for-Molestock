...
int EtoPPressure=0;
  ...
  //set_delta_constants(); //toto potencialni problem....
 
  //float delta_tower1_x = (delta_radius + tower_adj[0]) * cos((210 + tower_adj[3]) * PI/180); // front left tower
  //float delta_tower1_y = (delta_radius + tower_adj[0]) * sin((210 + tower_adj[3]) * PI/180); 
  //float delta_tower2_x = (delta_radius + tower_adj[1]) * cos((330 + tower_adj[4]) * PI/180); // front right tower
  //float delta_tower2_y = (delta_radius + tower_adj[1]) * sin((330 + tower_adj[4]) * PI/180); 
  //float delta_tower3_x = (delta_radius + tower_adj[2]) * cos((90 + tower_adj[5]) * PI/180);  // back middle tower
  //float delta_tower3_y = (delta_radius + tower_adj[2]) * sin((90 + tower_adj[5]) * PI/180);  
...
void setup_filrunoutpin() {
    ...
    pinMode(FILRUNOUT_PIN, INPUT); // nema byt pinMode(4, INPUT_PULLUP)?
...
void setup()
{
  setup_killpin();
  pinMode(4, INPUT);
  setup_filrunoutpin(); //TFS mod (added from new Marlin)
  setup_powerhold();
...
static void engage_z_probe() {
    ...
    #else // Deploy the Z probe by touching the belt, no servo needed. //TFs mod probe leveling (vysunuti sondy)
    feedrate = homing_feedrate[X_AXIS];
    destination[X_AXIS] = 20;
    destination[Y_AXIS] = 135;
    destination[Z_AXIS] = 100;
    prepare_move_raw();

    feedrate = homing_feedrate[X_AXIS]/10;
    destination[X_AXIS] = -7;
    prepare_move_raw();
    st_synchronize();
...
static void retract_z_probe() {
    ...
    #else // Push up the Z probe by moving the end effector, no servo needed. // TFs mod probe leveling push up (zasunuti sondy)
    feedrate = homing_feedrate[X_AXIS];
    destination[Z_AXIS] = current_position[Z_AXIS] + 40; //original 20 //TFs removed for tests
    //destination[X_AXIS] = 0;
    //destination[Y_AXIS] = 0;
    //destination[Z_AXIS] = 150;    
    prepare_move_raw();

    destination[X_AXIS] = -59.5; //-59.5; //-61.5;
    destination[Y_AXIS] = 141; //138.4; //143; //137.1;
    destination[Z_AXIS] = 40;
    //destination[X_AXIS] = -79.6;
    //destination[Y_AXIS] = 109.7;
    //destination[Z_AXIS] = 40;
    prepare_move_raw();
    ...
    destination[Z_AXIS] = current_position[Z_AXIS] - 22 ;//21.5;
    prepare_move_raw();
...
static void extrapolate_unprobed_bed_level() {
  //TFs mod
    ...
    //nacist a zpracovat ulozena data?
  #else
...
#define HOMEAXIS_DO(LETTER) \
  ...
  if (axis==X_AXIS ? HOMEAXIS_DO(X) :
    ...
    if (endstop_adj[axis] * axis_home_dir < 0) {
      enable_endstops(false);  // TFs mod add // Ignore Z probe while moving away from the top microswitch.
      plan_set_position(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], current_position[E_AXIS]);
      destination[axis] = endstop_adj[axis];
      plan_buffer_line(destination[X_AXIS], destination[Y_AXIS], destination[Z_AXIS], destination[E_AXIS], feedrate/60, active_extruder);
      st_synchronize();
      enable_endstops(true);  // TFs mod add // Stop ignoring Z probe while moving up to the top microswitch again.
    }
#endif
...
void set_delta_constants()
{
  ...
  //delta_tower1_x = (delta_radius + tower_adj[0]) * cos((210 + tower_adj[3]) * PI/180); // front left tower
  //delta_tower1_y = (delta_radius + tower_adj[0]) * sin((210 + tower_adj[3]) * PI/180); 
  //delta_tower2_x = (delta_radius + tower_adj[1]) * cos((330 + tower_adj[4]) * PI/180); // front right tower
  //delta_tower2_y = (delta_radius + tower_adj[1]) * sin((330 + tower_adj[4]) * PI/180); 
  //delta_tower3_x = (delta_radius + tower_adj[2]) * cos((90 + tower_adj[5]) * PI/180);  // back middle tower
  //delta_tower3_y = (delta_radius + tower_adj[2]) * sin((90 + tower_adj[5]) * PI/180); 
    ...
    case 29: // G29 Detailed Z-Probe, probes the bed at 3 or more points.
        {
            ...
            // TFs mod reset correction matrix
            #ifdef SAVE_G29_CORRECTION_MATRIX
              plan_bed_level_matrix.set_to_identity(); //Reset the plane ("erase" all leveling data)
            #endif            
...
Sigma_Exit:
    ...
    SERIAL_PROTOCOLLN(MSG_M119_REPORT);
      ...
      #endif
	  //TFs mod - filament runout status
      #if defined(FILRUNOUT_PIN) && FILRUNOUT_PIN > -1
        SERIAL_PROTOCOLPGM("Filament runout sensor: ");
        SERIAL_PROTOCOLLN(((READ(FILRUNOUT_PIN)^FIL_RUNOUT_INVERTING)?"filament present":"filament out"));
      #endif	  
      break;
    ...
    case 666: // M666 set delta endstop adjustemnt
	  ...
	  //TFs mod from Rich Cattell
	   ...
	   if (code_seen('L')) {
	     SERIAL_ECHOLN("Current Delta geometry adj:");
...
{
	 //TFs mod from Rich Cattell, radius ignored
	 set_delta_constants();
...
void manage_inactivity(bool ignore_stepper_queue/*=false*/)
{
  ...
  if(stepper_inactive_time)  {
	...
	//TFs mod (added from new Marlin)
	if( (millis() - previous_millis_cmd) >  stepper_inactive_time && !ignore_stepper_queue)
    {
...
#ifdef FILAMENT_RUNOUT_SENSOR
  void filrunout() {
    //SERIAL_ECHOLN("udalost - filament runout");
